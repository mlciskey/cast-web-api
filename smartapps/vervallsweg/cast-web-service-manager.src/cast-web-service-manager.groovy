...
def configureDevicePage(dni) {
	def thisDni = dni["dni"]
    if (thisDni == null) {
    	thisDni = state.configCurrentDevice
    }
    def d = getChildDevices()?.find { it.deviceNetworkId == thisDni }
    ...
    resetFormVar(d)
    logger('debug', "presetObject: " + d.getDataValue("presetObject"))
    ...
    dynamicPage(name: "configureDevicePage", title: "Configure "+d.deviceNetworkId, nextPage: "saveDeviceConfigurationPage") {
        section("Device settings") {
            ...
            input(name: "device_type", type: "enum", title: "Device type", defaultValue: [d.getDataValue("deviceType")], options: ["Chromecast", "Google Home", "Google Home Max", "Google Home Mini", "Group"], required: true)
        }
        ...
        section("Presets") {
            ...
            href(name: "presetGenerator2",title: "Preset generator",required: false,style: "external",url: "https://vervallsweg.github.io/smartthings/cast-web-preset-generator/preset-generator.html",description: "")
        }
...
def renderConfig() {
    def configString = "${apiServerUrl("/api/smartapps/installations/")}${app.id}/updatestatus/?access_token=${state.accessToken}"
    
    render contentType: "text/plain", data: configString
}

def deviceRefresh(description) {
	logger("debug", "params = ${params}")
    def ds = parseJson(params.devicestatus)
    def d = getChildDevices()?.find { it.deviceNetworkId == params.devicename }
	if (d != null) {   
    	d.parseReceiverStatus(ds.status)
    }
    
	render contentType: "text/plain", data:  "OK"
}

def updateStatus(description) {
	logger("debug", "updateStatus params = ${params}")
	...
	if (d != null) {
		...
		if (params.mediastatus) {
            def ms = parseJson(params.mediastatus)
            d.parseMediaStatus(ms.status)
        }   	
    }
...
mappings {
    ...
    path("/devicerefresh")		{ action: [GET: "deviceRefresh", POST: "deviceRefresh"]  }
    path("/updatestatus")		{ action: [GET: "updateStatus", POST: "updateStatus"]  }
}
