...
metadata {
    ...
    tiles(scale: 2) {
	    ...
	    standardTile("displayStatus", "device.displayStatus", width: 2, height: 2, canChangeIcon: false, decoration: "flat") {
            state("paused", label:"Paused", icon:"http://cdn.device-icons.smartthings.com/sonos/pause-icon@2x.png", action:"music Player.play", nextState: "playing")
            state("playing", label:"Playing", icon:"http://cdn.device-icons.smartthings.com/sonos/play-icon@2x.png", action:"music Player.pause", nextState: "paused", backgroundColor:"#00a0dc")
            state("Ready to cast", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlechomecast00.png")
            state("Ready to cast00", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlechromecast00.png")
            state("Ready to cast01", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlehome00.png")
            state("Ready to cast02", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlehomemax00.png")
            state("Ready to cast03", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlehomemini00.png")
            state("Ready to cast99", label:"Ready", icon: "https://raw.githubusercontent.com/mlciskey/public/master/smartthings/icons/googlegroup00.png")
            state("group", label:"Group", icon:"http://cdn.device-icons.smartthings.com/sonos/play-icon@2x.png", backgroundColor:"#00a0dc")
        }
...
def getDeviceType() {
    try {
        if(getDataValue('deviceType') == "Chromecast") {
        	return "00"
        }
        if(getDataValue('deviceType') == "Google Home") {
        	return "01"
        }
        if(getDataValue('deviceType') == "Google Home Max") {
        	return "02"
        }
        if(getDataValue('deviceType') == "Google Home Mini") {
        	return "03"
        }
        if(getDataValue('deviceType') == "Group") {
        	return "99"
        }
    }
    catch (e) {
...
def installed() {
    ...
    updateDataValue('deviceType', "Chromecast")
...
def updated() {
	...
	logger("debug", "status = ${device.currentValue("status")}, getDeviceType = ${getDeviceType()}")
    sendEvent(name: "displayStatus", value: device.currentValue("status"), displayed:false, changed: true)
    if( device.currentValue("status").equals("Ready to cast") ) {
        sendEvent(name: "displayStatus", value: 'Ready to cast' + getDeviceType(), displayed:false, changed: true)
	}
    parsePresets()
    restartPolling()
}
...
def playPreset(number) {
    ...
    JSONObject presets = new JSONObject( getDataValue("presetObject") )
    if( presets.get(key).mediaTitle == defaultMediaTitle ) {
...
def parseMediaStatus(mediaStatus) {
    ...
    if(mediaStatus.media) {
    	...
    	removeTrackData(['title', 'subtitle', 'preset', 'sessionId', 'displayName'])
    }
...
def parsePlayerState(playerState) {
    if( playerState[0] ) {
        ...
        if( playerState[0].toLowerCase().equals("playing") || playerState[0].toLowerCase().equals("paused") ) {
            ...
            sendEvent(name: "displayStatus", value: playerState[0].toLowerCase(), changed: true)
        }
    }
}
...
def setGroupPlayback(boolean group) {
    if(group) {
        ...
        sendEvent(name: "displayStatus", value: 'group', displayed:false, changed: true)
    } else {
...
def generateSwitchStatus() {
    logger('debug', "Executing 'generateSwitchStatus', ${device.currentValue('trackData')} length:" + device.currentValue('trackData').length() )
    ...
    if( device.currentValue('trackData').length()>2 && device.currentValue('status').equals("playing") ) { //TODO: handle 'group' status
        ...
        sendEvent(name: "displayStatus", value: "Ready to cast" + getDeviceType(), changed: true)
        sendEvent(name: "switch", value: off)
    }
}
...
def setMediaPlayback(mediaType, mediaUrl, mediaStreamType, mediaTitle, mediaSubtitle, mediaImageUrl) {
	if (!mediaImageUrl) {
    	mediaImageUrl = 'https://lh3.googleusercontent.com/nQBLtHKqZycERjdjMGulMLMLDoPXnrZKYoJ8ijaVs8tDD6cypInQRtxgngk9SAXHkA=w300'
    }
...
def setMediaPlaybackShort(mediaType, mediaUrl, mediaStreamType, mediaTitle, mediaSubtitle, mediaImageUrl) {
    ...
    sendEvent(name: "displayStatus", value: "playing", changed: true)
